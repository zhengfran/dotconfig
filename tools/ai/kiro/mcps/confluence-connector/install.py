#!/usr/bin/env python3
"""
Confluence MCP Server - GUI Installer for Kiro
...
This installer will:
  ...
  3. Ask for your Confluence base URL and Personal Access Token
...
INSTALL_SUBDIR = "confluence-mcp-server"
...
class InstallerApp:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("Confluence MCP Server - Installer")
    ...
    def _build_ui(self):
        ...
        # --- Title ---
        title = ttk.Label(main, text="Confluence MCP Server Installer", font=("Segoe UI", 14, "bold"))
        title.pack(anchor="w")
        subtitle = ttk.Label(main, text="Set up the Confluence MCP server for local use with Kiro.", font=("Segoe UI", 9))
        ...
        conf_frame = ttk.LabelFrame(main, text="Confluence Configuration", padding=10)
        conf_frame.pack(fill="x", pady=(0, 10))

        ttk.Label(conf_frame, text="Confluence Base URL:").pack(anchor="w")
        self.url_var = tk.StringVar(value="https://central.confluence.automotive.cloud")
        url_entry = ttk.Entry(conf_frame, textvariable=self.url_var, width=60)
        url_entry.pack(fill="x", pady=(0, 8))

        pat_label_row = ttk.Frame(conf_frame)
        pat_label_row.pack(fill="x")
        ttk.Label(pat_label_row, text="Personal Access Token (PAT):").pack(side="left")
        create_pat_btn = ttk.Button(pat_label_row, text="Create PAT in Confluence...", command=self._open_pat_page)
        create_pat_btn.pack(side="right")

        self.pat_var = tk.StringVar()
        pat_entry = ttk.Entry(conf_frame, textvariable=self.pat_var, show="*", width=60)
        pat_entry.pack(fill="x", pady=(0, 2))
        ttk.Label(conf_frame, text="Opens your Confluence profile page where you can create a token",
    ...
    def _open_pat_page(self):
        ...
        if not base_url:
            messagebox.showwarning("Confluence URL Required", "Please enter your Confluence base URL first.")
            return
        import webbrowser
    ...
    def _start_install(self):
        ...
        if not base_url:
            messagebox.showerror("Missing Field", "Please enter your Confluence base URL.")
            return
        if not base_url.startswith("http"):
            messagebox.showerror("Invalid URL", "Confluence URL should start with https://")
            return
        if not pat:
    ...
    def _do_install(self, install_dir: Path, base_url: str, pat: str):
        ...
        existing_config["mcpServers"]["confluence-connector"] = new_entry
        ...
        self.root.after(0, lambda: messagebox.showinfo(
            "Installation Complete",
            "Confluence MCP Server installed successfully!\n\n"
            "Restart Kiro to activate the confluence-connector MCP server.\n\n"
...
def check_python_early():
    """Fail fast with a message box if Python is too old."""
    v = sys.version_info
    if (v.major, v.minor) < MIN_PYTHON:
        try:
            root = tk.Tk()
