...
export function normalizeCopilotUsage(
  ...
  // Under token-based billing the legacy quota fields are placeholders --
  // unlimited=true, entitlement=0, percent_remaining=100 on every snapshot --
  // and reporting them would describe the account as an unlimited seat with no
  // usage. The only live figure is credits_used, so it becomes the one window.
  // GitHub does not report the credit entitlement here, so `limit` stays unset
  // and the consumer supplies the budget.
  if (root.token_based_billing === true) {
    const credits =
      finiteNumber(asRecord(snapshots.premium_interactions)?.credits_used) ??
      Object.values(snapshots)
        .map((entry) => finiteNumber(asRecord(entry)?.credits_used))
