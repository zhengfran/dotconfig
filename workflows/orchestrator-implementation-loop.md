...
The retained model preference is Claude Code Sonnet 5 at high effort with permission bypass. If the user later resumes the queue, new tickets use Sonnet 5 in fresh workspaces inside Herdr `default`; historical Terra conversations remain completed records.
...
Herdr topology updated on 2026-09-13: for future tickets, create a new workspace in the existing `default` session, with a fresh native Claude Code conversation. Do not create a separate Herdr session per ticket. Keep each already-running ticket in its recorded conversation through completion.
...
- For each future ticket, use Herdr `agent start` with `--kind claude` and native arguments `--model claude-sonnet-5 --effort high --dangerously-skip-permissions`. Verify the startup resolves to Claude Sonnet 5 with high effort and permission bypass. Do not silently substitute another model or a moving model alias if Sonnet 5 is unavailable. Installed Claude Code help supports the model, effort and permission flags; verify exact model availability at launch.
- For each new ticket, use explicit `herdr --session default` and create a new uniquely named workspace with `--no-focus`, a unique agent name, and a new native Claude Code conversation. Use the workspace/tab/pane IDs returned by creation; never assume `w1:p1` or target an existing user's pane. Do not create another Herdr server/session per ticket, or fork, resume, or reuse the preceding ticket's conversation. Record and verify model, effort, mode, native session ID, Herdr session, workspace/tab/pane IDs, agent name, branch and baseline. Historical Terra conversations are completed records, not candidates for new ticket work.
...
1. Read the current map and child frontmatter, the saved loop state, and any new user steering. Use local repository/vault continuation evidence while the saved Nowledge approval denial remains in force; do not retry or indirectly execute that rejected external lookup without new authorization evidence resolving it.
...
3. If no ticket is active under the stop-after-current instruction, pause the heartbeat; do not select or claim another ticket. Only after an explicit user resumption may the supervisor select the first open, unclaimed ticket by filename whose dependencies are all resolved, skipping documented external blockers. Preserve concurrent user changes and pin the accepted starting commit on an isolated branch or worktree as needed.
4. Atomically claim each new child note with a named Claude Code Sonnet 5 assignee before dispatch. Regenerate the minimap from child state, including its actual assignee. Save a temporary, self-contained handoff with the ticket path, accepted contracts, repository instructions, baseline, permitted scope, tests and delivery requirements. Historical claims and handoffs remain unchanged.
...
10. Mark the current implementation outcome in loop history, preserve unresolved operational evidence gates in the vault, and pause the heartbeat. Do not choose or start another ticket. A later explicit user resumption uses a new workspace in Herdr `default` and a fresh native conversation; never reuse a completed conversation for another ticket.
...
If the current ticket cannot proceed because of a documented external prerequisite, retain its implementation evidence and blocker without resolving it. Under the stop-after-current instruction, do not continue other independent tickets. Finish any authorized local preparation and bounded review, notify the user once, then pause the heartbeat. A failed model launch, usage limit or permission denial must be reported accurately, not worked around by changing the requested model or broadening scope.
