...
custom_pickers.project_files = function()
	...
	if is_inside_work_tree[cwd] then
		require("scripts.visible-path").prettyFilesPicker({ picker = "git_files" })
	else
		require("scripts.visible-path").prettyFilesPicker({ picker = "find_files" })
	end
end
			...
			attach_mappings = function(prompt_bufnr)
				action_set.select:replace(function()
					...
					require("telescope").extensions.egrepify.egrepify({
						search_dirs = dirs,
						layout_strategy = require("scripts.layout-strategy").grep_layout(),
					})
				end)
				return true
			end,
