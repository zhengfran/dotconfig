...
;; Keybinding: SPC b u (bookmark → URL)
(zzc/leader-keys
...
;; - Add: Quick-add command to append new bookmark to file
;; - Recent: Sort by access frequency/recency
;; - Multiple files: Search across multiple bookmark files

;; Validation helper: Check for plain-text URLs that need org-link format
(defun my/validate-bookmarks ()
  "Validate ~/org/bookmarks.org for plain-text HTTP(S) URLs.
Reports URLs that need to be wrapped in [[ ]] brackets for parsing.
Returns count of issues found (0 if all URLs properly formatted)."
  (interactive)
  (let* ((org-file (expand-file-name "~/org/bookmarks.org"))
         (issues '())
         (issue-count 0))
    (unless (file-exists-p org-file)
      (user-error "Bookmarks file not found: %s" org-file))
    
    (with-temp-buffer
      (insert-file-contents org-file)
      (goto-char (point-min))
      
      ;; Search for plain HTTP(S) URLs (not in org-link format)
      (while (re-search-forward "^[[:space:]]*\\(https?://[^[:space:]]+\\)[[:space:]]*$" nil t)
        (let ((url (match-string 1))
              (line-num (line-number-at-pos)))
          ;; Verify it's NOT already in org-link format
          (save-excursion
            (beginning-of-line)
            (unless (looking-at "^[[:space:]]*\\[\\[https?://")
              (push (cons line-num url) issues)
...
(zzc/leader-keys
  "bmv" '(my/validate-bookmarks :which-key "validate bookmarks"))
